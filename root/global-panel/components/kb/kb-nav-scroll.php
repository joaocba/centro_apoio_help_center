...
<nav id="navbar-example3" class="h-100 flex-column align-items-stretch border-end">
    ...
    foreach ($categorias as $row_cat) { //processo de iteração para cada categoria
        ...
        echo '<a class="nav-link" href="./global-panel/knowledge-base.php#item-' . $row_cat['id'] . '">' . $catNavCounter . ' - ' . $row_cat['nome_categoria'] . '</a>';
        ...
        foreach ($artigos as $row_art) { //processo de iteração para cada artigo associado à categoria da hierarquia
            echo '<a class="nav-link ms-3 my-1" href="./global-panel/knowledge-base.php#item-' . $cat_id_import . '-' . $row_art['id'] . '">' . $catNavCounter . '.' . $artNavCounter . ' - ' . $row_art['assunto'] . '</a>';
            $artNavCounter += 1;
        }
        echo '</nav>';
        $catNavCounter += 1;
